...
for e in kconfig_db["symbols"]:
    ...
    for k, v in listings:
        ...
        for i, d in enumerate(e[k]):
            if type(d) == str and (d.startswith("CONFIG") or d.startswith("SB_CONFIG")):
                d = f'<a href="#{d}">{d}</a>'
            if k == "alt_defaults":
...
for e in kconfig_db["symbols"]:
    ...
    html_str = re.sub(r"<li>(SB_CONFIG_.*?)<\/li>", option_to_href_li, html_str)
...
for id, name, t, path in res:
    if not (name.startswith("CONFIG") or name.startswith("SB_CONFIG")):
