...
void test_single_without_psk(void)
{
	...
	err = wifi_credentials_set_personal(SSID2, sizeof(SSID2), SECURITY2, BSSID2, 6,
					    PSK2, 0, FLAGS2);
	TEST_ASSERT_EQUAL(EXIT_SUCCESS, err);
...
/* Verify that we can handle SSIDs that contain NULL characters in unusual places. */
...
void test_storage_limit(void)
{
	...
	/* Set third network */
	err = wifi_credentials_set_personal(SSID3, sizeof(SSID3), SECURITY3, BSSID3, 6,
