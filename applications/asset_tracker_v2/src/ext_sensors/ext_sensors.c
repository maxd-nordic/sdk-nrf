					  ...
					  const struct sensor_trigger *trig)
{
	...
	case SENSOR_TRIG_THRESHOLD:
		...
		if ((abs(evt.value_array[0]) > threshold ||
			...
			evt.type = EXT_SENSOR_EVT_ACCELEROMETER_LP_TRIGGER;
			evt_handler(&evt);
		}
				   ...
				   const struct sensor_trigger *trig)
{
	...
	case SENSOR_TRIG_THRESHOLD:
		...
		if (evt.value > 0.0) {
			evt.type = EXT_SENSOR_EVT_ACCELEROMETER_HG_TRIGGER;
			evt_handler(&evt);
		}
		break;
	default:
...
int ext_sensors_init(ext_sensor_handler_t handler)
{
	...
	if (!device_is_ready(accel_sensor_lp.dev)) {
		...
		evt_handler(&evt);
	} else if (IS_ENABLED(CONFIG_EXTERNAL_SENSORS_ACTIVITY_DETECTION_AUTO)) {
		struct sensor_trigger trig = {
			.chan = SENSOR_CHAN_ACCEL_XYZ,
			.type = SENSOR_TRIG_THRESHOLD
		};

		int err = sensor_trigger_set(accel_sensor_lp.dev,
	...
	if (!device_is_ready(accel_sensor_hg.dev)) {
		...
		evt.type = EXT_SENSOR_EVT_ACCELEROMETER_ERROR;
		evt_handler(&evt);
	} else if (IS_ENABLED(CONFIG_EXTERNAL_SENSORS_IMPACT_DETECTION_AUTO)) {
		struct sensor_trigger trig = {
			.chan = SENSOR_CHAN_ACCEL_XYZ,
			.type = SENSOR_TRIG_THRESHOLD
		};
