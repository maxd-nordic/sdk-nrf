...
static void prov_get_status_handler(Request *req, Response *rsp)
{
	...
	memcpy(rsp->device_status.provisioning_info.ssid.bytes,
	       creds.header.ssid, creds.header.ssid_len);
	rsp->device_status.provisioning_info.ssid.size = creds.header.ssid_len;
	memcpy(rsp->device_status.provisioning_info.bssid.bytes, creds.header.bssid, 6);
	...
	rsp->device_status.provisioning_info.auth = creds.header.type;
...
static void prov_set_config_handler(Request *req, Response *rsp)
{
	...
	/* SSID */
	memcpy(creds.header.ssid, req->config.wifi.ssid.bytes, req->config.wifi.ssid.size);
	creds.header.ssid_len = req->config.wifi.ssid.size;
	/* BSSID */
	if (req->config.wifi.bssid.size != 6) {
		LOG_WRN("Expected BSSID len of 6, Actual len %d.", req->config.wifi.bssid.size);
	} else {
		memcpy(creds.header.bssid,
		       req->config.wifi.bssid.bytes, req->config.wifi.bssid.size);
	}
	/* Password and security */
	if (req->config.has_passphrase == true) {
		memcpy(creds.password, req->config.passphrase.bytes, req->config.passphrase.size);
		creds.password_len = req->config.passphrase.size;
		...
		if (req->config.wifi.has_auth == true) {
				if (req->config.wifi.auth == AuthMode_OPEN) {
					...
					creds.header.type = WIFI_SECURITY_TYPE_UNKNOWN;
				}
		}
	} else {
	...
	rc = wifi_credentials_set_personal_struct(creds.header.ssid, creds.header.ssid_len, &creds);
	if (rc) {
		LOG_ERR("Failed to store credentials: %d", rc);
	}
