			   ...
			   const char *const msg_type)
{
	...
	case NRF_CLOUD_OBJ_TYPE_JSON: {
		...
		err = (cJSON_AddStringToObjectCS(new_obj, NRF_CLOUD_JSON_APPID_KEY, app_id)
			       ? 0
			       : -ENOMEM);

		if (!err && msg_type) {
			err = (cJSON_AddStringToObjectCS(new_obj, NRF_CLOUD_JSON_MSG_TYPE_KEY,
							 msg_type)
				       ? 0
				       : -ENOMEM);
		}
...
int nrf_cloud_obj_ts_add(struct nrf_cloud_obj *const obj, const int64_t time_ms)
{
	...
	case NRF_CLOUD_OBJ_TYPE_JSON: {
		...
		return cJSON_AddNumberToObjectCS(obj->json, NRF_CLOUD_MSG_TIMESTAMP_KEY, time_ms)
			       ? 0
			       : -ENOMEM;
	}
	case NRF_CLOUD_OBJ_TYPE_COAP_CBOR: {
...
int nrf_cloud_obj_num_add(struct nrf_cloud_obj *const obj, const char *const key, const double val,
			  const bool data_child)
{
	...
	case NRF_CLOUD_OBJ_TYPE_JSON: {
		...
		return cJSON_AddNumberToObjectCS(dest_json_get(obj, data_child), key, val)
			       ? 0
			       : -ENOMEM;
	}
	case NRF_CLOUD_OBJ_TYPE_COAP_CBOR: {
			  ...
			  const char *const val, const bool data_child)
{
	...
	case NRF_CLOUD_OBJ_TYPE_JSON: {
		...
		return cJSON_AddStringToObjectCS(dest_json_get(obj, data_child), key, val)
			       ? 0
			       : -ENOMEM;
	}
	case NRF_CLOUD_OBJ_TYPE_COAP_CBOR: {
...
int nrf_cloud_obj_bool_add(struct nrf_cloud_obj *const obj, const char *const key, const bool val,
			   const bool data_child)
{
	if (!obj || !key) {
		return -EINVAL;
	...
	case NRF_CLOUD_OBJ_TYPE_JSON: {
		...
		return cJSON_AddBoolToObjectCS(dest_json_get(obj, data_child), key, val) ? 0
											 : -ENOMEM;
	}
	default:
		break;
				...
				const bool data_child)
{
	...
	case NRF_CLOUD_OBJ_TYPE_JSON: {
		...
		return cJSON_AddItemToObjectCS(dest_json_get(obj, data_child), key, array)
			       ? 0
			       : -ENOMEM;
	}
	default:
...
int nrf_cloud_obj_cloud_encode(struct nrf_cloud_obj *const obj)
{
	...
	case NRF_CLOUD_OBJ_TYPE_COAP_CBOR: {
		...
		ret = coap_codec_message_encode(obj->coap_cbor, (uint8_t *)obj->encoded_data.ptr,
				  ...
				  const struct nrf_cloud_gnss_data *const gnss)
{
	...
	msg_type =
		(IS_ENABLED(CONFIG_NRF_CLOUD_COAP) && (obj->type == NRF_CLOUD_OBJ_TYPE_COAP_CBOR))
			? NULL
					  ...
					  const struct nrf_cloud_location_config *const config)
{
	...
	if (config && ((config->do_reply != NRF_CLOUD_LOCATION_DOREPLY_DEFAULT) ||
		...
		err = nrf_cloud_obj_object_add(obj, NRF_CLOUD_LOCATION_JSON_KEY_CONFIG, &config_obj,
					       false);
		if (err) {
			goto cleanup;
		}
