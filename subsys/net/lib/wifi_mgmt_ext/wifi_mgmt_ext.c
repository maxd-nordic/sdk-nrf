...
static int add_network_from_credentials_struct_personal(struct wpa_supplicant *wpa_s,
...
static void add_stored_network(void *cb_arg, const char *ssid, size_t ssid_len)
{
	...
	add_network_from_credentials_struct_personal(wpa_s, &creds);
}

static int add_static_network_config(struct wpa_supplicant *wpa_s)
{
#if defined(CONFIG_WIFI_CREDENTIALS_STATIC_SSID)

	#warning "Static WiFi configuration is used, please remove before deployment!"
	...
	if (strcmp("OPEN", CONFIG_WIFI_CREDENTIALS_STATIC_TYPE) == 0) {
		creds.header.type = WIFI_SECURITY_TYPE_NONE;
	} else if (strcmp("WPA2-PSK", CONFIG_WIFI_CREDENTIALS_STATIC_TYPE) == 0) {
		creds.header.type = WIFI_SECURITY_TYPE_PSK;
	} else if (strcmp("WPA2-PSK-SHA256", CONFIG_WIFI_CREDENTIALS_STATIC_TYPE) == 0) {
		creds.header.type = WIFI_SECURITY_TYPE_PSK_SHA256;
	} else if (strcmp("WPA3-SAE", CONFIG_WIFI_CREDENTIALS_STATIC_TYPE) == 0) {
	...
	return add_network_from_credentials_struct_personal(wpa_s, &creds);
#endif /* defined(CONFIG_WIFI_CREDENTIALS_STATIC_SSID) */
	return 0;
}
			    ...
			    void *data, size_t len)
{
	...
	ret = add_static_network_config(wpa_s);
	if (ret) {
		goto out;
	}
