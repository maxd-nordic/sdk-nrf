						 ...
						 struct wifi_credentials_personal *buf)
{
	...
	if (idx == -1) {
		LOG_ERR("Cannot retrieve WiFi credentials, no entry found for the provided SSID");
		ret = -ENOENT;
		goto exit;
	}
...
int wifi_credentials_set_personal_struct(const char *ssid, size_t ssid_len,
					 const struct wifi_credentials_personal *buf)
{
	int ret;
	...
	ret = wifi_credentials_store_entry(idx, buf, sizeof(struct wifi_credentials_personal));
	...
	wifi_credentials_cache_ssid(idx, &buf->header);
				  ...
				  size_t password_len, uint32_t flags)
{
	...
	default:
		LOG_ERR("Cannot set WiFi credentials, "
			"provided security type %d is unsupported", type);
		return -ENOTSUP;
	}

	/* store entry */
	ret = wifi_credentials_set_personal_struct(ssid, ssid_len,
						   (struct wifi_credentials_personal *)header);
